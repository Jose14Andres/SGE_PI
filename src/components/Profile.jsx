...
export default function Profile({ user, onUpdateAvatar, onUpdateProfile, onChangePassword }) {
  ...
  const inputCls = "w-full bg-[#000000] border border-[#E2E8F0]/20 rounded-lg px-3 py-2 text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:border-[#FF0033] focus:ring-1 focus:ring-[#FF0033]";
  ...
  return (
    <div className="max-w-lg space-y-6">
      ...
      {/* Avatar */}
      <div className="bg-[#111111]/80 backdrop-blur-sm border border-[#E2E8F0]/20 rounded-2xl p-6 space-y-5">
        <div className="flex flex-col items-center gap-4">
          <div className="relative">
            {preview
              ? <img src={preview} alt="avatar" className="w-24 h-24 rounded-full object-cover border-4 border-[#FF0033]/40 shadow-lg shadow-[#FF0033]/20" />
              : <div className="w-24 h-24 rounded-full bg-[#0066FF]/20 border-4 border-[#FF0033]/40 flex items-center justify-center text-2xl font-bold text-[#FF0033] font-serif">
                  {user.nombre?.[0]}{user.apellido?.[0]}
                </div>
            }
            <button
              onClick={() => fileRef.current?.click()}
              className="absolute -bottom-1 -right-1 w-8 h-8 rounded-full bg-[#0066FF] border-2 border-[#111111] flex items-center justify-center text-[#F4F6F9] hover:brightness-110 transition-colors shadow-lg"
              title="Cambiar foto"
            >
          ...
          <button onClick={() => fileRef.current?.click()} className="text-sm text-[#FF0033] hover:text-[#F4F6F9] border border-[#FF0033]/20 hover:border-[#FF0033]/40 px-4 py-1.5 rounded-lg transition-all">
            Subir foto de perfil
          </button>
        </div>

        {preview && preview !== user.avatar && (
          <button onClick={handleSaveAvatar} className="w-full py-2.5 bg-gradient-to-r from-[#0066FF] to-[#FF0033] hover:brightness-110 active:scale-[0.98] focus-visible:ring-2 focus-visible:ring-[#FF0033] text-[#F4F6F9] rounded-xl text-sm font-medium transition-colors shadow-lg shadow-[#FF0033]/20">
      ...
      {/* Datos personales */}
      <div className="bg-[#111111]/80 backdrop-blur-sm border border-[#E2E8F0]/20 rounded-2xl p-6 space-y-4">
        <div className="flex items-center justify-between">
          ...
          {!editMode && (
            <button onClick={() => { setEditMode(true); setProfileError(''); setProfileFields({ nombre: user.nombre, apellido: user.apellido, email: user.email }); }}
              className="text-xs text-[#FF0033] hover:text-[#F4F6F9] border border-[#FF0033]/20 hover:border-[#FF0033]/40 px-3 py-1 rounded-lg transition-all">
              Editar
            </button>
          )}
        </div>
        ...
        {editMode ? (
          <div className="space-y-3">
            ...
            <div className="flex gap-2 pt-1">
              <button onClick={handleSaveProfile} className="flex-1 py-2 bg-gradient-to-r from-[#0066FF] to-[#FF0033] hover:brightness-110 active:scale-[0.98] focus-visible:ring-2 focus-visible:ring-[#FF0033] text-[#F4F6F9] rounded-lg text-sm font-medium transition-colors">
                Guardar cambios
              </button>
      ...
      <div className="bg-[#111111]/80 backdrop-blur-sm border border-[#E2E8F0]/20 rounded-2xl p-6 space-y-4">
        ...
        {pwMode ? (
          <div className="space-y-3">
            ...
            <div className="flex gap-2 pt-1">
              <button onClick={handleSavePassword} className="flex-1 py-2 bg-gradient-to-r from-[#0066FF] to-[#FF0033] hover:brightness-110 active:scale-[0.98] focus-visible:ring-2 focus-visible:ring-[#FF0033] text-[#F4F6F9] rounded-lg text-sm font-medium transition-colors">
