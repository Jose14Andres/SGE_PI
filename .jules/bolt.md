...
- **Optimizing repetitive array lookups in login flow**: When performing operations sequentially on the same record in an array (like checking role and then password), avoid repeating an `O(N)` `Array.prototype.find()` lookup if the object was already found. Retaining the reference to the found object and directly checking its properties (`found.password !== hashedPassword`) turns the second validation into an `O(1)` check.
